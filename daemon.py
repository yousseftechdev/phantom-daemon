...
WEB_HOST: str = settings.WEBUI_HOST
WEB_PORT: int = settings.WEBUI_PORT
WEB_DIR: str = settings.WEBUI_DIR
...
class TelemetryCollector:
    ...
    def _get_temperature(self) -> dict:
        try:
            ...
            package_temp = (
                round(sum(core_temps) / len(core_temps), 1)
                if core_temps
                else None
            )
...
async def register_client(websocket) -> None:
    ...
    print(
        f"[+] Client connected: {remote[0]}:{remote[1]} | Active: {len(CLIENTS)}"
    )
    try:
...
async def main() -> None:
    ...
    print(f"Web UI serving at http://{WEB_HOST}:{WEB_PORT}")
